...
const Products = () => {
    ...
    return (
        <div className='flex flex-col lg:flex-row container mx-auto gap-3 lg:gap-6'>
            ...
            <button className='btn rounded-4xl' onClick={()=>handleCategorySelect('all')}>All Products</button>
                 {
                    categories.map((category,idx)=><button className='btn rounded-4xl' key={idx} onClick={()=>handleCategorySelect(category)}>{category}</button>)
                 }
            </div>
